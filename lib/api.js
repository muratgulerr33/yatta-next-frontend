...
/**
 * Partner public profil bilgilerini getirir (SSR için Next.js native fetch kullanır)
 * @param {string} slug - Partner profil slug'ı
 * @returns {Promise<Object>} Partner profil verisi
 */
export async function fetchPartnerPublicProfile(slug) {
  const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.yatta.com.tr'
  const url = `${API_BASE_URL}/api/v1/partners/${slug}/`
  
  try {
    const res = await fetch(url, {
      // SSR için cache stratejisi: public profil → revalidate mantıklı
